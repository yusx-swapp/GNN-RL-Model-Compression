...
def load_model(model_name,data_root,device=None):
    ...
    if model_name == "resnet56":
        ...
        path = os.path.join('networks', "pretrained_models",'cifar10','resnet56.th')
        ...
        net = torch.nn.DataParallel(net)
        path = os.path.join('networks', "pretrained_models",'cifar10','resnet44.th')
        checkpoint = torch.load(path,map_location=device)
        net.load_state_dict(checkpoint['state_dict'])
        ...
        net = torch.nn.DataParallel(net)
        path = os.path.join('networks', "pretrained_models",'cifar10', 'resnet110.th')
        checkpoint = torch.load(path, map_location=device)
        net.load_state_dict(checkpoint['state_dict'])
        ...
        net = torch.nn.DataParallel(net)
        path = os.path.join('networks', "pretrained_models",'cifar10', 'resnet32.th')
        checkpoint = torch.load(path, map_location=device)
        net.load_state_dict(checkpoint['state_dict'])
        ...
        net = torch.nn.DataParallel(net)
        path = os.path.join('networks', "pretrained_models",'cifar10', 'resnet20.th')
        checkpoint = torch.load(path, map_location=device)
        net.load_state_dict(checkpoint['state_dict'])
